...
const App: React.FC = () => {
  ...
  // UI State for Chat vs Dashboard
  ...
  useEffect(() => {
    // Only scroll if we are in chat view
    if (isChatView) {
  ...
  const handleAgentSelect = (id: AgentId) => {
    if (id !== currentAgentId) {
        ...
        // Always reset to dashboard when manually switching agents
        setIsChatView(false); 
    }
  ...
  const handleUserSubmit = async (text: string) => {
    ...
    // Switch to chat view immediately upon interaction
    setIsChatView(true);
        ...
        <div className="flex-1 flex flex-col bg-[#0b101b]/80 backdrop-blur-xl border border-white/5 rounded-3xl overflow-hidden shadow-2xl relative min-w-0">
            ...
            <header className="h-24 flex items-center justify-between px-8 border-b border-white/5 bg-white/[0.02] shrink-0">
                ...
                <div className="flex items-center gap-4 md:gap-6">
                    ...
                    {/* History Toggle Button - Only show if there is history */}
                    {messages.length > 0 && (
                       <button 
                    ...
                    <div className="hidden md:flex items-center gap-3 px-4 py-1.5 rounded-full bg-black/40 border border-white/5">
            ...
            <div className="flex-1 overflow-y-auto relative scroll-smooth custom-scrollbar">
              {!isChatView ? (
                // Show Manager Dashboard by default (or if explicitly toggled)
                <ManagerDashboard 
                  agent={currentAgent} 
